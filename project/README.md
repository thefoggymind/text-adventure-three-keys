...
# 3つの鍵 — Web Edition（ブラウザ版）

テキストアドベンチャー「3つの鍵」のWebブラウザ版です。Pythonのインストール不要で、ブラウザ上で直接プレイできます。

## 遊び方

1. [公開URL](https://thefoggymind.github.io/three-keys-web) にアクセス（または後述のデプロイ手順で自身でホスティング）
2. 「はじめる」ボタンをクリック
3. 表示された選択肢（1〜4）をクリックして物語を進める
4. 全6エンディングを目指そう！
...
| 操作 | 方法 |
|------|------|
| 選択肢を選ぶ | 表示されたボタンをクリック / キーボードの `1`〜`4` キー |
| セーブ | `💾 セーブ` ボタンをクリック / `S` キー |
| ゲームを続ける | タイトル画面の「つづきから」ボタン（セーブデータがある場合のみ表示） |
| タイトルに戻る | エンディング画面の「タイトルに戻る」ボタン |
| テーマ切替 | ゲーム画面右上の `🌓` ボタン |
| 音量調整 | ブラウザの音量設定で調整 |
...
| 4 | **右の道 - 敗北** | 右の道で負ける（50%） | 呪いで石になってしまう |
| 5 | **中立「帰還」** | 「冒険をあきらめて帰る」を選ぶ | 危険を避け、平穏な日常に戻る |
| 6 | **隠し「真の英雄」** | 「周辺を探索」→「左/右の道」→洞窟探索 | 伝説の財宝を手にし、真の英雄となる |

### 隠しエンディングの手順

1. メニューで「4. 周辺を探索する」を選び、**錆びた鍵** と **古い地図** を入手する
2. 「1. 左の道を進む」または「2. 右の道を進む」を選ぶ
3. 隠し洞窟の入り口が現れたら「はい」を選ぶ

## 実績システム

全12個の実績を搭載しています。ゲーム中のさまざまな行動（ゲーム開始・選択肢・アイテム入手・エンディング到達）を自動検出し、条件を満たすと解除されます。
...
- **解除通知**: 実績解除時に画面上部にポップアップがフェードイン表示され、2.5秒後に自動で消えます（`showAchievementPopup()`）
- **一覧表示**: タイトル画面の「🏆 実績」ボタンで実績一覧を表示/非表示。解除済みの実績は名称・アイコンが表示され、`achievement-item unlocked` クラスで視覚的に区別されます
- **非表示実績**: 秘密（○）実績は解除するまで一覧に「???」と表示されます
- **エンディング進行度**: エンディング画面下部に「全6エンディング中 N/6 コンプリート」の進行度バーを表示（`showEndingScreen()`）
- **完全制覇シェア**: 全12実績コンプリート時、一覧下部に「🎉 完全制覇をシェア」ボタンが出現。クリックでX（Twitter）の投稿画面が開きます
- **データ保存**: 実績データはブラウザの `localStorage`（キー `three-keys-achievements-v1`）に保存。サーバーサイドは一切不要です
- **内部構造**: 検出ロジックは `game.js` の `checkAchievements(eventType, data, achievements)` で実装。イベント種別（`GAME_START`, `CHOICE_MADE`, `ENDING_REACHED`, `ITEM_FOUND`）ごとに条件判定・解除記録を行います

## ファイル構成

| ファイル | 説明 |
|----------|------|
| `index.html` | HTMLエントリポイント（寄付ボタン含む） |
| `style.css` | 全画面のスタイル定義（Light/Darkテーマ対応） |
| `renderer.js` | DOM操作・画面遷移・セーブ/ロードを担当 |
| `game.js` | ゲームロジック（状態管理・選択肢・結果判定） |
| `assets/favicon.svg` | ファビコン |
| `assets/ogp.svg` | OGP画像（SNSシェア用） |
| `tests/` | Jestテストファイル（165テスト） |
...
# ローカルサーバー起動
npx serve -p 3001
# → http://localhost:3001 でプレイ可能
```

## デプロイ手順（概要）

詳細は [DEPLOY.md](web_adventure/DEPLOY.md) を参照。

### GitHub Pages（無料）

1. リポジトリの Settings → Pages で `main` ブランチの `/web_adventure` フォルダを指定
2. 数分後、`https://<username>.github.io/<repository>/web_adventure/` で公開完了

### Netlify（無料）

1. [Netlify](https://app.netlify.com) にログイン → "Import from Git"
2. リポジトリを選択、Publish directory に `web_adventure` を指定
3. デプロイ完了後、`https://<site-name>.netlify.app` で公開

### Firebase Hosting（無料枠あり）

1. [Firebase Console](https://console.firebase.google.com) でプロジェクトを作成
2. Firebase CLI をインストール: `npm install -g firebase-tools`
3. `firebase login` で認証後、`web_adventure/` で `firebase init hosting` を実行
4. public ディレクトリに `web_adventure` を指定、SPA設定は「いいえ」
5. `firebase deploy` で公開完了 → `https://<project-id>.web.app` でアクセス可能

> **注意**: 本ゲームは静的ファイル（HTML/CSS/JS）のみで構成されているため、バックエンドサーバーは不要です。上記いずれの無料ホスティングでもそのまま動作します。
...
| **JavaScript** | 必須（ES2020+ 対応） |
| **localStorage** | セーブデータ・実績データの保存に使用 |
| **画面サイズ** | スマートフォン（320px〜）・タブレット・デスクトップに対応 |
| **追加ソフトウェア** | 不要（Python / サーバーサイド一切不要） |
| **ホスティング** | 静的ファイルホスティングのみで動作（GitHub Pages / Netlify 等） |
| **Node.js**（開発時のみ） | v18 以上推奨。テスト実行（`npx jest`）・ローカルサーバー（`npx serve`）・Firebase CLI（`firebase-tools`）に必要 |

## 収益化オプション

本ゲームは完全無料でプレイいただけますが、以下の方法で開発を支援できます。

### ☕ 寄付（Ko-fi）

エンディング画面・タイトル画面に **Ko-fi** への寄付リンクを設置済みです。
- URL: [https://ko-fi.com/thefoggymind](https://ko-fi.com/thefoggymind)
- リンクは `index.html` 内のプレースホルダーとして実装されており、実際の寄付先URLは人間が設定してください。

### 💰 収益化のアイデア（未実装）

| 方法 | 説明 | 実装の容易さ |
|------|------|------------|
| **Ko-fi / Buy Me a Coffee** | 任意の寄付リンクを設置（✅ 済み） | 簡単 |
| **広告表示** | Google AdSense 等の広告をフッターに追加 | 中程度 |
| **GitHub Sponsors** | リポジトリでスポンサー登録を受け付ける | 簡単 |
| **有料DLC（追加シナリオ）** | 追加エンディングや特別ストーリーを販売 | 難しい |
| **アフィリエイト** | 広告収入を得る | 中程度 |
...
「3つの鍵」のストーリーの魅力、実績システム、遊び方、今後のアップデート予定をまとめた紹介記事を公開しています。

👉 [**ゲーム紹介記事を読む**](web_adventure/INTRO_BLOG.md)